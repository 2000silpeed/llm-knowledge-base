...
| **PowerPoint (.pptx/.ppt)** | 멀티모달 2-패스: 텍스트 패스(python-pptx) + 이미지 패스(LibreOffice → PyMuPDF → Vision LLM 슬라이드 전체 분석), 발표자 노트 포함 |
...
- **PPT (멀티모달 2-패스):**
  - **텍스트 패스:** python-pptx로 각 슬라이드를 `## Slide N: 제목` 형식으로 변환. 임베드 이미지는 로컬 저장 + 1문장 캡션 생성. 발표자 노트는 `> Note:` 블록쿼트로 포함.
  - **이미지 패스 (선택):** LibreOffice headless로 PPTX → PDF 변환 → PyMuPDF로 슬라이드별 고해상도 PNG 렌더링 → Vision LLM이 슬라이드 전체(주제·텍스트·차트·다이어그램·강조)를 `### 시각 분석` 섹션으로 작성.
  - `ingest.slide_render: false`로 이미지 패스 비활성화 가능 (LibreOffice 미설치 환경 대응).
  - `vision_llm` 설정 블록으로 Vision 전용 모델(Ollama Gemma 등) 별도 지정 가능.
  - Vision 분석 실패 시: `kb retry-vision <md-파일>`로 이후 재실행 지원.
- **Word:** 스타일(제목1/2/3 → #/##/###) 계층 구조 보존. 표는 마크다운 테이블로. 각주/미주는 문서 하단에 모아서 변환. 트랙변경 내역은 기본 무시 (옵션으로 포함 가능).

**Office 파일 파싱 라이브러리:**
```
Excel:      openpyxl + xlrd (레거시 .xls)
PowerPoint: python-pptx (텍스트 패스) + LibreOffice + PyMuPDF (이미지 패스)
Word:       python-docx
...
PDF 파싱: pymupdf (fitz)
Office 파싱: openpyxl (Excel), python-pptx + PyMuPDF (PPT), python-docx (Word), xlrd (레거시 .xls)
Office 렌더링: LibreOffice headless (PPT → PDF 변환, 이미지 패스 활성화 시)
웹 클리핑: trafilatura + html2text
이미지 처리: Pillow + Claude Vision API (또는 Ollama Vision 모델)
CLI: typer + rich
```
...
### Phase 4 — 인제스터 고도화 및 운영 보조 ✅ 완료 (2026-04-17)

```
W1-04b: PPT 인제스터 멀티모달 2-패스 업그레이드
  - 텍스트 패스(python-pptx) + 이미지 패스(LibreOffice → PyMuPDF → Vision LLM) 분리
  - 슬라이드별 시각 분석 결과를 ### 시각 분석 섹션으로 조립
  - vision_llm 설정 블록으로 Vision 전용 모델(Ollama 등) 별도 지정 지원
  - ingest.slide_render: false로 이미지 패스 on/off 제어

W1-04c: PPT Vision 캡션 재실행 기능
  - kb retry-vision <md-파일>: Vision 분석이 누락된 슬라이드만 선택적 재실행
  - --slides 1,3,5-8: 특정 슬라이드 번호(범위) 지정 지원
  - --force: 기존 분석 덮어쓰기 / --dry-run: 대상 목록 확인

W6-01: 위키 삭제 프로세스
  - kb remove <raw-파일>: raw 파일 + 연관 wiki 개념 항목 통합 삭제
    * wiki/_index.md, _summaries.md에서 항목 자동 제거
    * 다른 개념 파일의 related_concepts frontmatter, ## 관련 개념 섹션 자동 정리
    * .meta.yaml, .kb_concepts/*.concepts.json 함께 삭제
  - kb wiki delete <개념명>: wiki concept만 삭제 (raw 유지)
  - --dry-run / --force / --wiki-only / --no-index / --no-backlinks 옵션 지원
```
